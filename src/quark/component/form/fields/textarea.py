"""
此模块定义了 TextArea 组件，使用 pydantic 进行数据验证和序列化。
...
class Component(BaseModel):
    """
    表示 TextArea 组件的类，使用 pydantic 进行数据验证和序列化。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "textAreaField"。
        ...
        addon_after (Optional[Any]): 带标签的 input，设置后置标签。
        addon_before (Optional[Any]): 带标签的 input，设置前置标签。
        allow_clear (bool): 可以点击清除图标删除内容，默认值为 False。
        auto_size (Optional[Any]): 自适应内容高度，可设置为 true | false 或对象：{ minRows: 2, maxRows: 6 }，默认值为 None。
        bordered (bool): 是否有边框，默认值为 True。
        default_value (Optional[Any]): 默认的选中项，默认值为 None。
        disabled (bool): 禁用，默认值为 False。
        ...
        size (Optional[str]): 控件大小。注：标准表单内的输入框大小限制为 middle，large | middle | small，默认值为 None。

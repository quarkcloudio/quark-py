...
class Component(BaseModel):
    """
    表示文件上传组件的类，使用 pydantic 定义组件属性和方法。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "fileField"。
        row_props (Optional[Dict[str, Any]]): 开启 grid 模式时传递给 Row 的属性。
        col_props (Optional[Dict[str, Any]]): 开启 grid 模式时传递给 Col 的属性。
        secondary (bool): 是否为次要控件，默认为 False。
        colon (bool): 是否显示 label 后的冒号，默认为 True。
        extra (Optional[str]): 额外提示信息。
        ...
        label (Optional[str]): label 标签文本。
        label_align (str): 标签文本对齐方式，默认为 "right"。
        label_col (Optional[Any]): label 标签布局。
        name (Optional[str]): 字段名。
        no_style (bool): 是否无样式，默认为 False。
        required (bool): 是否必填，默认为 False。
        tooltip (Optional[str]): label 旁提示信息。
        value_prop_name (Optional[str]): 子节点值属性名。
        wrapper_col (Optional[Any]): 输入控件布局样式。
        column (TableColumn): 列表和详情页列属性。
        align (str): 列对齐方式。
        fixed (Optional[Any]): 列是否固定。
        editable (bool): 表格列是否可编辑，默认为 False。
        ellipsis (bool): 是否自动缩略，默为 False。
        copyable (bool): 是否支持复制，默为 False。
        filters (Optional[Any]): 表头筛选菜单项。
        order (int): 查询表单权重，默为 0。
        sorter (Optional[Any]): 可排序列。
        span (int): 详情页包含列数量，默为 0。
        column_width (int): 列表页列宽，默为 0。
        ...
        show_on_creation (bool): 是否在创建页展示，默为 True。
        show_on_update (bool): 是否在编辑页展示，默为 True。
        show_on_export (bool): 是否在导出 Excel 展示，默为 True。
        show_on_import (bool): 是否在导入 Excel 展示，默为 True。
        callback (Optional[Any]): 回调函数。
        default_value (Optional[Any]): 默认选中的选项。
        disabled (bool): 整组是否失效，默认为 False。
        value (Optional[Any]): 指定选中项。
        button (str): 上传按钮标识，默认为 "上传文件"。
        limit_size (int): 上传文件大小限制，默认为 2。
        limit_type (List[str]): 上传文件类型限制，默认为 ["jpeg", "png", "doc", "docx"]。
        limit_num (int): 上传文件数量限制，默认为 3。
    ...
    api: Optional[str] = "/api/admin/upload/file/handle"
    ignore: bool = False
    rules: List[Rule] = []
    ...
    def init(self):
        ...
        self.limit_size = 2
        self.limit_num = 3
        self.limit_type = ["jpeg", "png", "doc", "docx"]
        self.api = "/api/admin/upload/file/handle"

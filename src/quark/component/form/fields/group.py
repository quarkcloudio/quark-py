...
class Component(BaseModel):
    """
    表示表单字段组的组件类，使用 pydantic 进行数据验证和管理。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "groupField"。
        ...
        show_on_index (bool): 在列表页展示，默认 False。
        show_on_detail (bool): 在详情页展示，默认 False。
        show_on_creation (bool): 在创建页面展示，默认 True。
        show_on_update (bool): 在编辑页面展示，默认 True。
        show_on_export (bool): 在导出的 Excel 上展示，默认 False。
        show_on_import (bool): 在导入 Excel 上展示，默认 False。
        callback (Optional[Any]): 回调函数。
        title (Optional[str]): 分组标题。
        body (Optional[Any]): 组件内容。
        size (int): 子元素个数，默认 32。
    """
    component_key: str = ""

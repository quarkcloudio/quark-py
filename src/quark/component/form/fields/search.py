"""
此模块实现了搜索组件，使用 pydantic 进行数据验证和序列化。
包含搜索组件的定义、初始化及各种配置方法。
"""
import json
...
class Component(BaseModel):
    """
    表示搜索组件的类，使用 pydantic 进行数据验证和序列化。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "searchField"。
        ...
        placeholder (str): 占位符，默认值为 "请输入要搜索的内容"。
        default_value (Optional[Any]): 默认选中的选项。
        disabled (bool): 整组失效，默认值为 False。
        options (List[Option]): 可选项数据源。
        option_type (Optional[str]): 用于设置 Search options 类型 default | button。
        size (Optional[str]): 大小，只对按钮样式生效, large | middle | small。

"""
此模块定义了 Image 组件类，使用 pydantic 进行数据验证和管理，
模拟 Go 代码中 Image 组件的功能和逻辑。
"""
import json
...
class Component(BaseModel):
    """
    表示 Image 字段的组件类，使用 pydantic 进行数据验证和管理。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "imageField"。
        ...
        mode (str): 上传模式，单图或多图，single|multiple。
        button (str): 上传按钮标识。
        limit_size (int): 上传文件大小限制。
        limit_type (List[str]): 上传文件类型限制。
        limit_num (int): 上传文件数量限制。
        limit_wh (Dict[str, int]): 上传图片宽高限制。
    """
    component_key: str = ""
    ...
    def init(self):
        ...
        self.limit_size = 2
        self.limit_num = 3
        self.limit_type = ["image/jpeg", "image/png"]
        self.api = "/api/admin/upload/image/handle"
        self.limit_wh = {"width": 0, "height": 0}

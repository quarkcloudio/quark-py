"""
此模块定义了 Space 组件类，用于表单和表格中的布局和字段管理。
使用 pydantic 进行数据验证和序列化，提供了丰富的配置和方法。
"""
import json
...
class Component(BaseModel):
    """
    表示 Space 组件的类，使用 pydantic 进行数据验证和序列化。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "spaceField"。
        ...
        align (str): 对齐方式，start | end | center | baseline，默认值为 ""。
        direction (str): 间距方向，默认值为 ""。
        size (str): 间距大小，默认值为 ""。
        split (str): 设置拆分，默认值为 ""。
        wrap (bool): 是否自动换行，仅在 horizontal 时有效，默认值为 False。
        body (Optional[Any]): 组件内容，默认值为 None。
    """
    component_key: str = ""

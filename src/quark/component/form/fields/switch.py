"""
此模块定义了 SwitchField 组件，使用 pydantic 进行数据验证和序列化。
...
class Component(BaseModel):
    """
    表示 SwitchField 组件的类，使用 pydantic 进行数据验证和序列化。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "switchField"。
        ...
        checked (bool): 指定当前是否选中，默认值为 False。
        checked_children (Optional[Any]): 选中时的内容。
        class_name (Optional[str]): Switch 器类名。
        default_checked (bool): 初始是否选中，默认值为 False。
        default_value (Optional[Any]): 默认选中的选项。
        disabled (bool): 整组失效，默认值为 False。
        loading (bool): 加载中状态，默认值为 False。
        size (Optional[str]): 选择框大小。
        un_checked_children (Optional[Any]): 自定义的选择框后缀图标。

"""
此模块定义了使用 pydantic 的表单字段集组件类，
包含组件属性和方法，用于构建和配置表单字段集。
"""
import json
import string
...
class Component(BaseModel):
    """
    表示表单字段集组件的类，使用 pydantic 定义组件属性和方法。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "fieldsetField"。
    ...
    component: str = Field("fieldsetField", description="组件名称")

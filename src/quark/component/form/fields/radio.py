...
class Component(BaseModel):
    """
    表示单选按钮组件的类，使用 pydantic 进行数据验证和序列化。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "radioField"。
        ...
        button_style (Optional[Any]): RadioButton 的风格样式，目前有描边和填色两种风格 outline | solid。
        default_value (Optional[Any]): 默认选中的选项。
        disabled (bool): 整组失效，默认值为 False。
        options (List[Option]): 可选项数据源。
        option_type (Optional[str]): 用于设置 Radio options 类型 default | button。

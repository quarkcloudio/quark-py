"""
此模块实现了 Transfer 组件，用于处理数据穿梭选择功能。
包含组件的初始化、属性设置、验证规则管理等功能。
"""
import json
...
class Component(BaseModel):
    """
    表示 Transfer 组件的类，使用 pydantic 进行数据验证和序列化。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "transferField"。
        ...
        data_source (List[DataSource]): 数据源，其中的数据将会被渲染到左边一栏中，target_keys 中指定的除外。
        disabled (bool): 是否禁用，默认值为 False。
        selections_icon (Optional[Any]): 自定义下拉菜单图标。
        filter_option (Optional[Any]): 根据搜索内容进行筛选，接收 inputValue option 两个参数，
            当 option 符合筛选条件时，应返回 true，反之则返回 false。
        footer (Optional[Any]): 底部渲染函数。
        list_style (Optional[Dict[str, Any]]): 两个穿梭框的自定义样式。
        locale (Optional[Dict[str, str]]): 各种语言,{ itemUnit: 项, itemsUnit: 项, searchPlaceholder: 请输入搜索内容 }。
        one_way (bool): 展示为单向样式，默认值为 False。
        operations (List[str]): 操作文案集合，顺序从上至下。
        operation_style (Optional[Dict[str, Any]]): 操作栏的自定义样式。
        pagination (Optional[Any]): 使用分页样式，自定义渲染列表下无效。
        select_all_labels (Optional[Any]): 自定义顶部多选框标题的集合。
        selected_keys (List[str]): 设置哪些项应该被选中。
        show_search (bool): 是否显示搜索框，默认值为 False。
        show_select_all (bool): 是否展示全选勾选框，默认值为 False。
        status (Optional[str]): 设置校验状态,'error' | 'warning'。
        target_keys (List[str]): 显示在右侧框数据的 key 集合。
        titles (List[str]): 标题集合，顺序从左至右。

"""
此模块定义了日期时间范围选择器组件相关的类和方法。
包含 `Component` 类及其相关的初始化和配置方法，用于构建和管理日期时间范围选择器组件。
"""
...
class Component(BaseModel):
    """
    表示日期时间范围选择器组件的类，包含组件的各种属性和配置方法。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "datetimeRangeField"。
        ...
        allow_clear (bool): 是否支持清除，默为 True。
        auto_focus (bool): 是否自动获取焦点，默为 False。
        bordered (bool): 是否有边框，默为 True。
        class_name (Optional[str]): 自定义类名。
        default_value (Optional[Any]): 默认的选中项。
        disabled (Optional[Any]): 是否禁用。
        format (str): 日期格式，默为 "YYYY-MM-DD HH:mm:ss"。
        popup_class_name (Optional[str]): 额外的弹出日历 className。
        input_read_only (bool): 输入框是否只读，默为 False。
        locale (Optional[Any]): 国际化配置。
        mode (Optional[str]): 日期面板的状态。
        next_icon (Optional[Any]): 自定义下一个图标。
        open (bool): 浮层是否显隐，默为 False。
        picker (Optional[str]): 选择器类型。
        placeholder (List[str]): 输入框占位文本，默为 ["开始时间", "结束时间"]。
        placement (Optional[str]): 浮层预设位置。
        popup_style (Optional[Any]): 额外的弹出日历样式。
        prev_icon (Optional[Any]): 自定义上一个图标。
        size (Optional[str]): 输入框大小。
        status (Optional[str]): 校验状态。
        ...
        show_now (bool): 面板是否显示“此刻”按钮，默为 False。
        show_time (Optional[Any]): 是否增加时间选择功能。
        show_today (bool): 是否展示“今天”按钮，默为 False。
    """
    component_key: str

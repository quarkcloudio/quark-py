"""
此模块定义了短信验证码组件，用于创建和配置短信验证码输入字段。
...
class Component(BaseModel):
    """
    表示短信验证码组件的类，使用 pydantic 进行数据验证和序列化。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "smsCaptchaField"。
        ...
        has_feedback (bool): 配合 valiTextStatus 属性使用，展示校验状态图标，
        ...
        addon_after (Optional[Any]): 带标签的 input，设置后置标签，默认值为 None。
        addon_before (Optional[Any]): 带标签的 input，设置前置标签，默认值为 None。
        allow_clear (bool): 可以点击清除图标删除内容，默认值为 False。
        bordered (bool): 是否有边框，默认值为 True。
        default_value (Optional[Any]): 默认的选中项，默认值为 None。
        disabled (Optional[Any]): 禁用，默认值为 None。
        id (Optional[str]): 输入框的 id，默认值为 None。
        max_length (int): 最大长度，默认值为 200。
        show_count (bool): 是否展示字数，默认值为 False。
        status (Optional[str]): 设置校验状态,'error' | 'warning'，默认值为 None。
        prefix (Optional[Any]): 带有前缀图标的 input，默认值为 None。
        size (Optional[str]): 控件大小。注：标准表单内的输入框大小限制为 middle，可选 large | middle | small，默认值为 None。
        suffix (Optional[Any]): 带有后缀图标的 input，默认值为 None。
        type (Optional[str]): 声明 input 类型，同原生 input 标签的 type 属性，默认值为 None。
        value (Optional[Any]): 指定选中项,string[] | number[]，默认值为 None。
        placeholder (str): 占位符，默认值为 "请输入"。
        style (Optional[Dict[str, Any]]): 自定义样式，默认值为 None。

        captcha_id_url (Optional[str]): 获取验证码ID的URL，默认值为 None。
        captcha_url (Optional[str]): 获取验证码URL，默认值为 None。
        dependency (str): 依赖的字段名称，默认为 "phone"。
        captcha_props (Optional[Captcha]): 验证码属性，默认值为 None。
    """
    component_key: str = ""
    ...
    def init(self):
        ...
        """
        self.component = "smsCaptchaField"
        self.colon = True

"""
此模块定义了 ID 组件类，使用 pydantic 进行数据验证和管理，
模拟 Go 代码中 ID 组件的功能和逻辑。
"""
import json
...
class Component(BaseModel):
    """
    表示 ID 字段的组件类，使用 pydantic 进行数据验证和管理。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "idField"。
        ...
        on_index_displayed (bool): 在列表页显示字段，默认 True。
        on_detail_displayed (bool): 在详情页显示字段，默认 True。
        on_form_displayed (bool): 在表单页显示控件，默认 True。
        on_export_displayed (bool): 在导出页显示字段，默认 True。
    """
    component_key: str = ""

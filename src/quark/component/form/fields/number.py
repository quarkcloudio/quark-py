"""
此模块使用 pydantic 定义了 Number 组件类，用于表示数字输入组件的相关信息。
"""
import json
...
class Component(BaseModel):
    """
    表示数字输入组件的类，使用 pydantic 进行数据验证和序列化。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "inputNumberField"。
        ...
        controls (bool): 是否显示增减按钮，也可设置自定义箭头图标，默认值为 False。
        decimal_separator (Optional[str]): 小数点，默认值为 None。
        default_value (Optional[Any]): 默认的选中项。
        disabled (Optional[Any]): 禁用。
        keyboard (bool): 是否启用键盘快捷行为，默认值为 False。
        max (Optional[int]): 最大值。
        min (Optional[int]): 最小值。
        precision (Optional[int]): 数值精度，配置 formatter 时会以 formatter 为准。
        read_only (bool): 只读，默认值为 False。
        status (Optional[str]): 设置校验状态,'error' | 'warning'。
        prefix (Optional[Any]): 带有前缀图标的 input。
        size (Optional[str]): 控件大小。注：标准表单内的输入框大小限制为 middle，large | middle | small。
        step (Optional[Any]): 每次改变步数，可以为小数。
        string_mode (bool): 字符值模式，开启后支持高精度小数。同时 onChange 将返回 string 类型，默认值为 False。
        value (Optional[Any]): 指定选中项,string[] | number[]。
        placeholder (str): 占位符，默认为 "请输入"。
    ...
    def init(self):
        ...
        """
        self.component = "inputNumberField"
        self.colon = True

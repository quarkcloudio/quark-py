"""
此模块定义了 Time 组件，使用 pydantic 进行数据验证和序列化。
包含组件初始化、属性设置和状态检查等功能。
"""
import json
...
class Component(BaseModel):
    """
    表示 Time 组件的类，使用 pydantic 进行数据验证和序列化。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "timeField"。
        ...
        class_name (Optional[str]): 自定义类名，默认值为 None。
        clear_icon (Optional[Any]): 自定义的清除图标，默认值为 None。
        clear_text (Optional[str]): 清除按钮的提示文案，默认值为 None。
        default_value (Optional[Any]): 默认的选中项，默认值为 None。
        disabled (Optional[Any]): 禁用，默认值为 None。
        disabled_time (Optional[Any]): 不可选择的时间，默认值为 None。
        format (Optional[str]): 设置日期格式，为数组时支持多格式匹配，展示以第一个为准，默认值为 None。
        hide_disabled_options (bool): 隐藏禁止选择的选项，默认值为 False。
        hour_step (Optional[int]): 小时选项间隔，默认值为 None。
        input_read_only (bool): 设置输入框为只读（避免在移动设备上打开虚拟键盘），默认值为 False。
        minute_step (Optional[int]): 分钟选项间隔，默认值为 None。
        open (bool): 控制浮层显隐，默认值为 False。
        placeholder (Optional[str]): 输入框占位文本，默认值为 "请选择"。
        placement (Optional[str]): 浮层预设位置，bottomLeft bottomRight topLeft topRight，默认值为 None。
        popup_class_name (Optional[str]): 额外的弹出日历 className，默认值为 None。
        popup_style (Optional[Any]): 额外的弹出日历样式，默认值为 None。
        second_step (Optional[int]): 秒选项间隔，默认值为 None。
        show_now (bool): 面板是否显示“此刻”按钮，默认值为 False。
        size (Optional[str]): 输入框大小，large | middle | small，默认值为 None。
        status (Optional[str]): 设置校验状态，'error' | 'warning'，默认值为 None。
        suffix_icon (Optional[Any]): 自定义的选择框后缀图标，默认值为 None。
        use_12_hours (bool): 使用 12 小时制，为 True 时 format 默认为 h:mm:ss a，默认值为 False。
        style (Optional[Dict[str, Any]]): 自定义样式，默认值为 None。
        value (Optional[Any]): 指定选中项,string[] | number[]，默认值为 None。
    """
    component_key: str = ""

"""
此模块使用 pydantic 定义了表单展示字段相关的组件类和方法。
包含 Component 类及其相关初始化和配置方法，用于构建和管理展示字段组件。
"""
import json
import string
...
class Component(BaseModel):
    """
    表示展示字段组件的类，使用 pydantic 定义组件的各种属性和配置方法。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "displayField"。
    ...
    component: str = Field("displayField", description="组件名称")
    ...
    default_value: Optional[Any] = Field(None, description="默认的选中项", alias="defaultValue")

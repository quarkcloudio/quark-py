"""
此模块定义了 TimeRange 组件，使用 pydantic 进行数据验证和序列化。
...
class Component(BaseModel):
    """
    表示 TimeRange 组件的类，使用 pydantic 进行数据验证和序列化。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "timeRangeField"。
        ...
        format (Optional[str]): 设置日期格式，为数组时支持多格式匹配，展示以第一个为准，默认值为 "HH:mm"。
        hide_disabled_options (bool): 隐藏禁止选择的选项，默认值为 False。
        hour_step (int): 小时选项间隔，默认值为 0。
        input_read_only (bool): 设置输入框为只读（避免在移动设备上打开虚拟键盘），默认值为 False。
        minute_step (int): 分钟选项间隔，默认值为 0。
        open (bool): 控制浮层显隐，默认值为 False。
        placeholder (List[str]): 输入框占位文本，默认值为 ["开始时间", "结束时间"]。

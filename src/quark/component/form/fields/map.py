"""
此模块定义了 MapField 组件类，使用 pydantic 进行数据验证和管理，
模拟 Go 代码中 MapField 组件的功能和逻辑。
"""
import json
...
class Component(BaseModel):
    """
    表示 MapField 字段的组件类，使用 pydantic 进行数据验证和管理。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "mapField"。
        row_props (Optional[Dict[str, Any]]): 开启 grid 模式时传递给 Row 的属性，
            仅在 ProFormGroup、ProFormList、ProFormFieldSet 中有效，默认：{ "gutter": 8 }。
        col_props (Optional[Dict[str, Any]]): 开启 grid 模式时传递给 Col 的属性，默认：{ "xs": 24 }。
        secondary (bool): 是否是次要控件，只针对 LightFilter 下有效，默认 False。
        colon (bool): 配合 label 属性使用，表示是否显示 label 后面的冒号，默认 True。
        extra (Optional[str]): 额外的提示信息，和 help 类似，
            当需要错误信息和提示文案同时出现时，可以使用这个。
        has_feedback (bool): 配合 validateStatus 属性使用，展示校验状态图标，
            建议只配合 Input 组件使用，默认 False。
        help (Optional[str]): 提示信息，如不设置，则会根据校验规则自动生成。
        hidden (bool): 是否隐藏字段（依然会收集和校验字段），默认 False。
        initial_value (Optional[Any]): 设置子元素默认值，如果与 Form 的 initialValues 冲突则以 Form 为准。
        label (Optional[str]): label 标签的文本。
        label_align (str): 标签文本对齐方式，默认为 "right"。
        label_col (Optional[Any]): label 标签布局，同 <Col> 组件，
            设置 span offset 值，如 { "span": 3, "offset": 12 } 或 { "sm": { "span": 3, "offset": 12 } }。
            可以通过 Form 的 labelCol 进行统一设置，不会作用于嵌套 Item。当和 Form 同时设置时，以 Item 为准。
        name (Optional[str]): 字段名，支持数组。
        no_style (bool): 为 True 时不带样式，作为纯字段控件使用，默认 False。
        required (bool): 必填样式设置。如不设置，则会根据校验规则自动生成，默认 False。
        ...
        wrapper_col (Optional[Any]): 需要为输入控件设置布局样式时，使用该属性，用法同 labelCol。
            可以通过 Form 的 wrapperCol 进行统一设置，不会作用于嵌套 Item。当和 Form 同时设置时，以 Item 为准。
        ...
        editable (bool): 表格列是否可编辑，只在列表页中有效，默认 False。
        ellipsis (bool): 是否自动缩略，只在列表页、详情页中有效，默认 False。
        copyable (bool): 是否支持复制，只在列表页、详情页中有效，默认 False。
        filters (Optional[Any]): 表头的筛选菜单项，当值为 true 时，自动使用 valueEnum 生成，只在列表页中有效。
        order (int): 查询表单中的权重，权重大排序靠前，只在列表页中有效，默认 0。
        sorter (Optional[Any]): 可排序列，只在列表页中有效。
        span (int): 包含列的数量，只在详情页中有效，默认 0。
        column_width (int): 设置列宽，只在列表页中有效，默认 0。
        ...
        show_on_index (bool): 在列表页展示，默认 True。
        show_on_detail (bool): 在详情页展示，默认 True。
        show_on_creation (bool): 在创建页面展示，默认 True。
        show_on_update (bool): 在编辑页面展示，默认 True。
        show_on_export (bool): 在导出的 Excel 上展示，默认 True。
        show_on_import (bool): 在导入 Excel 上展示，默认 True。
        callback (Optional[Any]): 回调函数。
        default_value (Optional[Any]): 默认选中的选项。
        disabled (bool): 整组失效，默认 False。
        style (Dict[str, Any]): 自定义样式，默认包含高度、宽度和上边距。
        value (Optional[Any]): 指定选中项,string[] | number[]。
        zoom (int): 缩放级别，默认 14。
        map_key (str): 地图 Key，默认 "70ac74a1443326e66e51a4255700a4e2"。
        map_security_js_code (str): 地图安全密钥，默认 "5c4fc57d6cba5efd1c15c988d18d2a78"。
    """
    component_key: str = ""
    ...
    def init(self):
        ...
        self.zoom = 14
        self.map_key = "70ac74a1443326e66e51a4255700a4e2"
        self.map_security_js_code = "5c4fc57d6cba5efd1c15c988d18d2a78"
        self.style = {
            "height": 500,
            "width": "100%",
            "marginTop": "10px"

"""
此模块定义了 List 组件类，使用 pydantic 进行数据验证和管理，
模拟 Go 代码中 List 组件的功能和逻辑。
"""
import json
...
class Component(BaseModel):
    """
    表示 List 字段的组件类，使用 pydantic 进行数据验证和管理。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "listField"。
        ...
        items (Optional[Any]): 列表项。
        button_text (str): 按钮文字，默认 "添加一行数据"。
        button_position (str): 按钮位置，默认 "top"。
        always_show_item_label (bool): Item 中总是展示 label，默认 True。
    """
    component_key: str = ""
    ...
    def init(self):
        ...
        self.button_text = "添加一行数据"
        self.button_position = "top"
        self.always_show_item_label = True

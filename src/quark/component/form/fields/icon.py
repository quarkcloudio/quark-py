"""
此模块定义了 Icon 组件类，使用 pydantic 进行数据验证和管理，
模拟 Go 代码中 Icon 组件的功能和逻辑。
"""
import json
...
class Component(BaseModel):
    """
    表示 Icon 字段的组件类，使用 pydantic 进行数据验证和管理。

    Attributes:
        component_key (str): 组件标识。
        component (str): 组件名称，默认为 "iconField"。
        ...
        size (str): 大小，large | middle | small，默认 ""。
        allow_clear (bool): 是否支持清除，默认 True。
        allow_search (bool): 是否支持搜索，默认 True。
        options (List[str]): 可选项数据源。
    """
    component_key: str = ""
    ...
    def set_allow_clear(self, val: bool):
        """
        设置是否支持清除。
        ...
        Returns:
            Component: 返回当前实例，支持链式调用。
        """
        self.allow_clear = val
        return self

    def set_allow_search(self, val: bool):
        """
        设置是否支持搜索。
